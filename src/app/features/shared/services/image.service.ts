...
export class ImageService {
    ...
    private api = inject(ApiService).for(environment.imagesApiBaseUrl);

    update(payload: FormData, id: string): Observable<ImageUpdateResponse> {
      return this.api.put<ImageUpdateResponse, FormData>(`${id}`, payload);
    }

    upload(payload: FormData): Observable<ImageUploadResponse> {
      return this.api.post<ImageUploadResponse, FormData>('', payload);
    }

    search(request: ImageSearchRequest): Observable<ImageSearchResponse> {
      return this.api.get<ImageSearchResponse, ImageSearchRequest>('search', request);
    }

    get(id: string): Observable<ImageGetResponse> {
      return this.api.get<ImageGetResponse>(`${id}`);
    }

    getPins(imageId: string): Observable<ImageGetPinsResponse> {
      return this.api.get<ImageGetPinsResponse>(`${imageId}/pins`);
    }

    addPin(imageId: string, pin: Pin): Observable<AddPinResponse> {
      return this.api.post<AddPinResponse, AddPinRequest>(`${imageId}/pins`, { pin });
    }

    updatePin(imageId: string, pin: Pin): Observable<UpdatePinResponse> {
      return this.api.put<UpdatePinResponse, UpdatePinRequest>(`${imageId}/pins/${pin.id}`, { pin });
    }

    deletePin(imageId: string, pinId: string): Observable<DeletePinResponse> {
      return this.api.delete<DeletePinResponse>(`${imageId}/pins/${pinId}`);
    }
}
